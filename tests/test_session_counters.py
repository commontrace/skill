...
Counter tests drive session_report._session_counters — per-session aggregates sent
...
from tests.base import HookTestCase, append_event, local_store, retrieval
...
class TestSessionCounters(HookTestCase):
    ...
    def test_empty_session_returns_zeros(self):
        conn = self.get_conn()
        self.assertEqual(
            session_report._session_counters(conn, self.state_dir, None),
    ...
    def test_fired_and_consumed_scoped_to_this_session(self):
        ...
        local_store.record_trigger(conn, "other-session", "bash_error")
        counters = session_report._session_counters(conn, self.state_dir, None)
    ...
    def test_assisted_counts_attributed_resolutions_in_window(self):
        ...
        counters = session_report._session_counters(conn, self.state_dir, pid)
...
class TestProvenanceFormatting(unittest.TestCase):
    ...
    def test_missing_contributor_name_omits_by(self):
        self.assertNotIn(" by ", retrieval.format_results(
