...
FIRST_RUN_NOTICE = (
    ...
    "summary (no transcripts, no files). Reading and /recall search are open "
    "to everyone and your work is always captured locally; PUBLISHING a trace "
    "to the shared commons, however, requires an invitation code — an "
    "un-invited anonymous account gets an HTTP 403 with an invitation notice "
    "when it tries to publish (redeem a code via POST "
    "/api/v1/invitations/redeem). Choose 'Always' at the contribution prompt "
    "to enable passive auto-contribution, or set auto_contribute to true in "
